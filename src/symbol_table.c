...
char* get_var_type(no* var_node,char* func_name, char* func_params){
	...
	aux_tab = symtab;
	while(aux_tab != NULL){ //correr a tabela global (variaveis globais e metodos)
		
		if( !strcmp(aux_tab->name, var_node->info->val) && aux_tab->params_list == NULL ){ //se é uma variavel global e tem o mesmo nome da que procuramos
...
void check_two_part_op(no* node,char* func_name, int isLogical, char* func_params){
	...
	if( op_type1 != NULL && op_type2 != NULL && !strcmp(op_type1,op_type2) ){ //se ambos os operandos têm o mesmo tipo
		...
		if( !strcmp(op_type1,"undef") && !strcmp(op_type2,"undef")){ //error: se os tipos forem undef, raise error
			...
			if(isLogical == 1){
				...
				if( !strcmp(node->tipo,"Or") || !strcmp(node->tipo,"And")){ //se for or ou and, tem de ter expressao bolean em cada filho
					...
					if(!strcmp(op_type1,"String[]")){ //TODO void??
						printf("Line %d, col %d: Operator %s cannot be applied to types %s, %s\n", node->info->line, node->info->col, get_node_operator(node->tipo), op_type1, op_type2);
					}
...
void check_one_part_op(no* node, char* func_name, int isLogical, char* func_params){
	...
	if( op_type != NULL && strcmp(op_type, "undef") && strcmp(op_type, "void")){ //não é null nem undef nem void
...
void check_call(no* node, tab_element* elem, char* func_name, char* func_params){
	...
	if(return_type == NULL){ //não existe nenhuma funcao com os mesmos parametros, temos de ir verificar se existe com parametros compatíveis
